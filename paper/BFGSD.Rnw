...
design with \Sexpr{length(n)} equally-spaced analyses.
Section~\ref{sec:canonical} will be explain how the type-I error rate of
sequential Bayes factor designs can be evaluated. We can see that the value and
shape of the critical value boundaries differs across the different Bayes
...
even up to ten analyses. In many fields, designs involving such a large number
of analyses are rarely encountered. For example, the most common number of
interim analyses in clinical trials is only one or two, if any
\citep{Stevely2015}.
...
sequential Bayes factor design. Assuming allocation of $n$ patients are
allocated to each group, the approximate standard deviation of the estimated log
odds ratio $\hat{\theta}$ is $\sigma = \sqrt{1/\{n \pi_0 (1 - \pi_0)\} + 1/\{n
  \pi_1 (1 - \pi_1)\}}$. Once data are observed, it is estimated through the
standard error obtained by plugging in the estimated rates $\hat{\pi}_0$ and
$\hat{\pi}_1$ \citep{Bland2000}. Assuming a normal distribution for
$\hat{\theta}$ around the true log odds ratio $\theta$ with standard deviation
$\sigma$, implies a canonical distribution for $z = \hat{\theta}/\sigma$ with
information level $I = 1/\sigma^2$. We can therefore apply the results from
...
decreasing with larger maximum sample sizes. In sum, compared to a fixed design,
a sequential design with even only two analyses can drastically lower the
expected sample size and increase the probability of correct evidence, while
only slightly increasing the probability of misleading evidence.
...
preclinical experiment with rats, which was retrospectively reported and
analyzed by \citet{Kang2025}. In the experiment, rats were randomly assigned to
different dose levels of a candidate drug or to a control group to estimate the
drug's effect on weight loss. Information that could be used to identify the
drugs or studies has been removed by \citet{Kang2025} due to confidentiality
issues. The top plot of Figure~\ref{fig:ratdata} shows the measured weight loss
values across groups. As can be seen, the low dose group (yellow) differs little
from the control group (black), whereas the medium (blue) and high dose (blue)
groups show substantially higher weight loss.
...
from the data and used to obtain the Bayes factor. Note that the order in which
the data were collected is unknown, which is why a random permutation is shown
here. Appendix~\ref{app:rats} shows that also for other permutations of the
...
competing hypotheses, and, unlike tail probabilities, there is no need to
