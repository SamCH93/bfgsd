...
\begin{abstract}
  \noindent The Bayes factor, the data-based updating factor from prior to
  posterior odds, is a principled measure of relative evidence for two competing
  hypotheses. It is naturally suited to sequential data analysis in settings
  such as clinical trials and animal experiments, where early stopping for
  efficacy or futility is desirable. However, designing such studies is
  challenging because computing design characteristics, such as the probability
  of obtaining conclusive evidence or the expected sample size, typically
  requires computationally intensive Monte Carlo simulations, as no closed-form
  or efficient numerical methods exist. To address this issue, we extend results
  from classical group sequential design theory to sequential Bayes factor
  designs. The key idea is to derive Bayes factor stopping regions in terms of
  the \textit{z}-statistic and use the known distribution of the cumulative
  \textit{z}-statistics to compute stopping probabilities through multivariate
  normal integration. The resulting method is fast, accurate, and
  simulation-free. We illustrate it with examples from clinical trials, animal
  experiments, and psychological studies. We also provide an open-source
  implementation in the \texttt{bfpwr} R package. Our method makes exploring
  sequential Bayes factor designs as straightforward as classical group
  sequential designs, enabling experiments to rapidly design informative and
...
probabilities \citep[see e.g.,][]{berry2010bayesian, Rosner2020} and Bayes
factors. Here we focus on sequential Bayes factor designs, which have been
applied across a wide range of domains, including biomedical research
\citep{Cornfield1976, Spiegelhalter2004b, Goodman2005, Johnson2009, Li2017,
  Zhu2019, Zhou2021, Rosner2021, Pourmohamad2022, Linde2023}, the social
sciences \citep{Schoenbrodt2017, Schoenbrodt2018, Stefan2019, Mani2021,
...
sequential designs based on likelihood ratios \citep{Wald1947, Siegmund1985,
  Royall1997}.
...
of the data's marginal likelihood under each hypothesis. Thus, the hypothesis
...
to a sequential version of the Bayesian \textit{t}-test used in psychological
science \citep{Gronau2020}. The paper closes with concluding discussions,
limitations, and opportunities for future research
(Section~\ref{sec:discussion}). Appendix~\ref{app:package} illustrates usage of
our R package \texttt{bfpwr} for planning sequential Bayes factor designs.
...
interest assess the usefulness of a design:
...
Using elementary rules of probability, the probability of finding evidence for
$H_1$ is
{\small
\begin{align*}
  \small
...
$$\mathrm{Var}(n) = \mathrm{E}(n^2) - \mathrm{E}(n)^2.$$ Each of these
quantities thus consists of a (weighted) sum of analysis-wise stopping
probabilities. These stopping probabilities are generally hard to compute due to
the accumulating data having a complex distribution with dependence across
analyses. For this reason, Monte Carlo simulation is typically used for
approximating design characteristics. However, in the following we will show
that by assuming a particular form of Bayes factor and data, fast and
simulation-free computation is possible.

\section{\textit{Z}-statistic based Bayes factors}
...
possible. These are related to a general class of test statistic based Bayes
factors \citep{Johnson2005, HeldOtt2018}. Assume now that we are interested in
the critical \textit{z}-values(s) for which $\mathrm{BF}_{01} = k$ for some $k >
0.$ For many Bayes factors in Table~\ref{tab:BFcritical}, critical values can be
derived in closed-form (see the right column).
...
null hypothesis ($\mathrm{BF}_{01} = 1/\Sexpr{1/k1}$). The critical values from
two commonly used group sequential design (GSD) methods, Pocock and
O'Brien-Fleming, are shown as comparison. Both ensure control of the type-I
error rate at the conventional $\alpha = 0.025$ (one-sided) in a sequential
design with \Sexpr{length(n)} equally-spaced analyses. We can see that the value
and shape of the critical value boundaries differs across the different Bayes
factors. The directional null ($H_0 \colon \theta \leq 0$) vs. directional
alternative ($H_1 \colon \theta > 0$) Bayes factor (black) shows near constant
critical values across all analyses. In contrast, the point null ($H_0 \colon
\theta = 0$) vs. point alternative ($H_1 \colon \theta = \Sexpr{pm1}$) Bayes
factor shows decreasing critical values (yellow), while the point null ($H_0
\colon \theta = 0$) vs. directional alternative ($H_1 \colon \theta > 0$) Bayes
factor shows increasing critical values (light-blue). The constant Pocock GSD
critical values (green) thus align with the shape of the directional null vs.
directional alternative Bayes factor (black) whereas the O'Brien-Fleming GSD
critical values (dark-blue) align with the shape of the point null vs. point
alternative Bayes factor (yellow).
\begin{figure}[!htb]
...
\subsection{\textit{Z}-statistic stopping regions}
...
computational challenges, we did not observe any numerical issues when
exhaustively computing the probability of all hyper-rectangles in designs with
even up to ten analyses. In many fields, carrying out so many analyses is
unrealistic. For example, one or two interim analyses are by far the most common
number of analyses in sequential clincial trials \citep{Stevely2015}.
Nevertheless, for designs with lots of analyses, it is often sufficient to
compute the probability of only the two hyper-rectangles where the sign of the
$z$-statistics stays constant across analyses. The probability of the remaining
hyper-rectangles is usually very small as it is unlikely that the $z$-statistic
will repeatedly change its sign because of the correlation of $z$-statistics
across analyses.
...
In contrast to the analysis prior, which is typically only
``weakly-informative'' or ``objective'' in some sense, the design prior should
represent genuine knowledge and uncertainty at the design analysis in order to
accurately estimate stopping probabilities and sample sizes. However, setting
the design prior to a point mass ($\tau_d \downarrow 0$) allows us to study the
frequentist operating characteristics of the sequential design. For example,
specifying a point prior at $\mu_d = 0$ and computing the probability to find
evidence for $H_1$ gives the frequentist type-I error rate. Despite the fact
that this probability may be unrealistic from a Bayesian perspective, showing
that a sequential Bayes factor design is appropriately calibrated (i.e., has
type-I error rate below a conventional level, e.g., 5\%) may be required from
certain stakeholders, such as a regulatory authorities in drug development
\citep{FDA2010, Campbell2020}.

Assuming there are $m$ analyses, the probability of evidence for $H_i \in \{H_0, H_1\}$ is
\begin{align*}
  \Pr(\text{Evidence for}~ H_i) =
  \sum_{j=1}^m \Pr\{\boldsymbol{Z}_{1:j} \in \boldsymbol{S}_j \mid \mu_d \boldsymbol{I}_{1:j},
  (\boldsymbol{\Sigma} + \tau_d^2 \boldsymbol{I} \boldsymbol{I}^\top)_{1:j,1:j}\}
\end{align*}
where $1:j$ indicates indexing of the first $j$ elements and $\boldsymbol{S}_j$
is the set of $z$-statistic stopping regions at analysis $j$. For example, assuming that $m = 3$ and that there
...
\begin{align*}
  &\boldsymbol{S}_1 = [z_{1,\text{crit}}(k_1), +\infty]&
  &\boldsymbol{S}_2 =
  \begin{bmatrix}
...
could help low-risk polycythaemia vera (Low-PV) patients kepping haematocrit
...
= \Sexpr{round(100*p0H1, 1)}\%$ and $\pi_1 = \Sexpr{round(100*p1H1, 1)}\%$ in
the control and treatment groups, corresponding to an odds ratio of $\mathrm{OR}
= \Sexpr{round(ORH1, 1)}$. The study was stopped after the second interim
analysis because the group sequential stopping bounds for efficacy were crossed.
Table~\ref{tab:lowpv} summarizes the by-analysis results.

\begin{table}[!htb]
  \centering
  \caption{Results from the Low-PV trial \citep{Barbui2021}. Shown are
    by-analysis sample sizes ($n$) and estimated probabilities ($\pi$) in
    control (subscript 0) and treatment groups (subscript 1), respectively,
...
contrasting $H_0 \colon \theta = 0$ to $H_1 \colon \theta = \log(\Sexpr{ORH1}) =
\Sexpr{round(log(ORH1), 2)}$. This leads to a Bayes factor $\mathrm{BF}_{01}^1 =
1/\Sexpr{round(1/bf1, 1)}$ in the first analysis and $\mathrm{BF}_{01}^2 =
1/\Sexpr{round(1/bf2, 1)}$ in the second. Had we used a threshold of $k_1 =
1/10$, the study would have thus been stopped for $H_1$ after the second
analysis, resulting in the same decision as was made by the trial investigators
based on a classical group sequential design.

Suppose the trial has not been carried out and we want to plan it using a
sequential Bayes factor design. Assuming allocation of $n$ patients to the
control and $n$ patients to the treatment group, the approximate standard error
of the estimated log odds ratio $\hat{\theta}$ is $\sigma = \sqrt{1/\{n \pi_0 (1
  - \pi_0)\} + 1/\{n \pi_1 (1 - \pi_1)\}}$, see Appendix~\ref{app:selogOR}.
Thus, assuming a canonical distribution for $z = \hat{\theta}/\sigma$ with
information level $I = 1/\sigma^2$, we can apply the results from
Section~\ref{sec:zstatistic} to compute sequential Bayes factor design
characteristics.
...
($\theta = \log(\Sexpr{ORH1}) = \Sexpr{round(log(ORH1), 2)}$, left plot) or
under $H_0$ ($\theta = 0$, right plot). Three equally-spaced analyses at 25, 50,
and 75 patients per group are considered, as in the original trial. We can see
that the probabilities of conclusive evidence for the corresponding true
hypotheses are very similar under both hypotheses. Both surpass 80\% after the
third analysis, but remain below 90\%. To achieve a probability of 90\%, the
maximum sample size and/or the number of interim analyses must be further
increased.
...
to determine the maximum sample size size to achieve a probability of conclusive
evidence of \Sexpr{round(100*pow, 1)}\%. This leads to $n_3 =
\Sexpr{ceiling(nmax0)}$ under $H_0$ and $n_3 = \Sexpr{ceiling(nmax1)}$ under
$H_1$. Thus taking the maximum of the two would ensure that the trial would
produce conclusive evidence with high probability.
...
top plots, we can see that the probability of obtaining ``correct'' evidence
...
producing misleading evidence and stopping for the wrong hypothesis
(upper-middle plot).

The probability of misleading evidence (i.e., $\mathrm{BF}_{01} \geq \Sexpr{k0}$
if $H_1$ is true and $\mathrm{BF}_{01} \leq 1/\Sexpr{1/k1}$ if $H_0$ is true)
first increases and then decreases again with increasing maximum sample size.
Since $H_0$ and $H_1$ are point hypotheses, the probability of misleading
evidence is bounded by $k = 1/10$, which is known as the ``universal bound''
\citep{Royall1997}. However, across all choices of the number of analyses, the
probability is much lower than the bound, remaining below 5\% across all maximum
sample sizes and number of analysises, though bringing it below 2.5\% (the
conventional type-I error rate for one-sided tests) requires substantial
increases in the maximum sample size for designs with many analyses. Finally,
increasing the number of analyses decreases the expected sample size
(middle-bottom plot), while it has a non-monotone effect on the standard
deviation of the sample size -- increasing for smaller maximum sample sizes and
decreasing with larger maximum sample sizes. In sum,
% %in the design of the Low-PV trial,
compared to a fixed design, a sequential design with even only two analyses can
drastically lower the expected sample size and increase the probability of
correct evidence, while only slightly increasing the probability of misleading
evidence.
%% In practice, the challenge is to find a combination of maximum sample size
%% and number of analyses that can be implemented and produces the desired
%% probabilities of correct evidence and expected sample size.
...
preclinical experiment with rats that was previously analyzed by
\citet{Kang2025}. In this experiment, rats were randomly assigned to different
dose levels of a candidate drug or to a control group to estimate the drug's
effect on weight loss. The top plot of Figure~\ref{fig:ratdata} shows the
measured weight loss values across groups. As can be seen, the low dose group
(yellow) differs little from the control group (black), whereas the medium
(blue) and high dose (blue) groups show substantially higher weight loss.
...
contrasting $H_0 \colon \theta_i = 0$ to $H_1 \colon \theta_i = 5$ is shown in
the bottom plot of Figure~\ref{fig:ratdata} for each treatment group (color). An
observation from each group is added at each step until the maximum group size
is reached. Note that the order in which the data were collected is unknown,
which is why a random permutation is shown here. Appendix~\ref{app:rats} shows
that also for other permutations of the data, similar results are obtained. As
the sample size increases, the Bayes factor in the low dose group (yellow)
increases and reaches $\mathrm{BF}_{01} > 10$ after 10 rats, suggesting strong
evidence for the absence of an effect over its presence. In contrast, the Bayes
factors in the medium (blue) and high (green) dose groups decrease with
increasing sample size, surpassing the threshold for strong evidence of
$\mathrm{BF}_{01} < 1/10$ after 12 and 8 rats, respectively. Thus, if the
experiment had been conducted using this sequential Bayes factor design,
\Sexpr{ratssaved$controlsaved} rats from the control group and
...
the standard error of the estimate (plot panels). Such a design prior can be
...
\Sexpr{pm}$), a ``replication Bayes factor'' \citep{VerhagenWagenmakers2014,
  Ly_etal2018, PawelHeld2022} would be obtained. However, here we will only
consider testing the same alternative hypotheses as in the original study.
...
1/\Sexpr{1/k1}$, blue curve) quickly increase with increasing sample size. They
...
actual number of observations/pairs for one-sample/paired $t$-tests, or the
harmonic sum of the group sizes for the two-sample $t$-tests), and
...
``Jeffreys-Zellner-Siow'' (JZS) Bayes factor \citep{Jeffreys1939, Zellner1980}.
...
\textit{t}-test, one of the most important sequential Bayes factor tests used in
practice.
...
Bayes factors offer an intuitive framework for sequential data analysis: Data
can be analyzed repeatedly without concerns about multiplicity, and stopping
decisions are naturally linked to interpretable updates of prior beliefs.
Despite these advantages, the broader adoption of sequential Bayes factor
designs has been limited, perhaps because of the difficulty of computing their
design characteristics. Existing approaches typically rely on extensive
simulation, which can be computationally costly, sensitive to Monte Carlo error,
and inconvenient when exploring many design options.
...
Under the canonical \textit{z}-statistic distribution, these regions can be
integrated efficiently using multivariate normal probabilities, eliminating the
need for simulation. The resulting computations are fast, accurate, and scalable
to designs with many interim looks. The approach also naturally incorporates
design priors, enabling experiments to account for parameter uncertainty in the
design. Traditional fixed parameter calculations are a special case, thus
enabling flexible exploration of both Bayesian and frequentist operating
characteristics within a unified framework.
...
avenue for future research. Furthermore, while multivariate normal integration
is highly efficient for realistic numbers of interim analyses, extremely large
numbers of looks remain computationally challenging. Nevertheless, such designs
are logistically challenging and rarely used in practice. For example, the most
common number of interim analyses in clinical trials is only one or two, if any
\citep{Stevely2015}.
...
We thank Wong Tsz Keung, Riko Kelter, and František Bartoš for valuable comments
...
openly available at \url{https://github.com/SamCH93/XXX}. A snapshot of the
...
\section{Standard error of the log odds ratio}
\label{app:selogOR}
